...
function nextAction(trial: Trial) {
  ...
  if (days !== null && days < 0)
    return {
      label: "Check if this opportunity was rescheduled",
      icon: ShieldAlert,
      tone: "text-amber-600",
    };
  if (days !== null && days <= 7)
    return {
      label: "Confirm details and prepare your kit",
      icon: CalendarCheck,
      tone: "text-primary",
    };
  if (trial.spots <= 30)
    return { label: "Review eligibility and apply early", icon: Clock3, tone: "text-primary" };
...
export function ParticipationAgent() {
  ...
  const pendingActions = useMemo(
    () => savedOpportunities.filter((trial) => !reminders[trial.id]).length,
    [savedOpportunities, reminders],
  );
    ...
    <Card className="border-primary/30 bg-gradient-card">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="font-semibold">Participation Agent</h2>
              <Badge className="border-0 bg-primary/15 text-primary">Beta</Badge>
            </div>
            <p className="mt-1 text-sm text-muted-foreground">
              I check your saved opportunities and turn them into practical next steps.
            </p>
          </div>
        </div>
      ...
      <CardContent className="pt-5">
        {savedOpportunities.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border p-6 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              The agent can remind you to verify details, prepare documents, and apply before the
              opportunity becomes easy to miss.
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className={`mt-1 flex items-center gap-1.5 text-xs ${action.tone}`}>
                        <ActionIcon className="h-3.5 w-3.5" />
                        {action.label}
                      </p>
                    </div>
                    <Button
                      onClick={() => scheduleCheckIn(trial)}
                      disabled={checkedIn}
                      size="sm"
                    ...
                    >
                      {checkedIn ? (
                        <>
                          <CheckCircle2 className="h-4 w-4" /> Check-in added
                        </>
                      ) : (
                        <>
                          <Send className="h-4 w-4" /> Check in with me
                        </>
                      )}
                    </Button>
                  </div>
        ...
        <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
          Check-ins are saved in this browser during the beta. The agent never guarantees selection
          and always points you back to the organizer&apos;s official details.
        </p>
      </CardContent>
    </Card>
  );
