  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          ...
          {
            name: "description",
            content: `Live ${loaderData.sport.name} trials, academies & scouts in ${loaderData.city.name}, ${loaderData.city.state}. ${loaderData.sport.tagline}.`,
          },
          { property: "og:title", content: `${loaderData.sport.name} in ${loaderData.city.name}` },
          {
            property: "og:description",
            content: `${loaderData.sport.tagline} — open trials across ${loaderData.city.name}.`,
          },
        ]
      : [],
  }),
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 sm:py-14">
      <nav className="text-xs text-muted-foreground">
        <Link to="/sports" className="hover:text-foreground">
          Sports
        </Link>
        {" / "}
        <Link to="/sport/$slug" params={{ slug: sport.slug }} className="hover:text-foreground">
          {sport.name}
        </Link>
        {" / "}
        <Link to="/city/$slug" params={{ slug: city.slug }} className="hover:text-foreground">
          {city.name}
        </Link>
      </nav>

      <header className="mt-4 flex items-start gap-4">
        <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-gradient-hero text-2xl">
          {sport.emoji}
        </div>
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            <MapPin className="inline h-4 w-4" /> {city.state} · {city.venueNote} · {all.length}{" "}
      ...
      <section className="mt-8 rounded-2xl border border-border bg-card/50 p-6">
        <h2 className="text-lg font-semibold">
          How to use this {sport.name} and {city.name} page
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          This page groups KhelGrid listings by sport and city. Read each opportunity detail page,
          check its source status, and confirm the organizer&apos;s current notice before travelling
          or paying.
        </p>
      </section>
      ...
      <section className="mt-8 rounded-2xl border border-border bg-secondary/40 p-6">
        <h2 className="text-lg font-semibold">
          Nearby {sport.name} hubs in {city.name}
        </h2>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {SPORTS_CATALOG.filter((s) => s.slug !== sport.slug)
            .slice(0, 8)
            .map((s) => (
              <Link
                key={s.slug}
