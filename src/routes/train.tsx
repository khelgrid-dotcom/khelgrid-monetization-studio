    ...
    () =>
      COACHING.filter(
        (c) =>
          ...
          (q === "" ||
            [c.title, c.coach, c.area].join(" ").toLowerCase().includes(q.toLowerCase())),
      ),
    [q, sport, city, level],
  );
    ...
    <main className="mx-auto max-w-7xl px-4 py-6 sm:py-10">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Structured coaching programs from certified academies near you.
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-[1fr,160px,160px,160px]">
          <div className="relative col-span-2 sm:col-span-1">
            ...
            <Input
              ...
              className="h-11 pl-9 bg-secondary/40 border-border"
            />
          </div>
          <Select value={city} onValueChange={setCity}>
            <SelectTrigger className="h-11 bg-secondary/40 border-border">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {list.map((c) => (
          <div
            key={c.id}
            className="overflow-hidden rounded-2xl border border-border bg-gradient-card transition-all hover:-translate-y-0.5 hover:border-border/80"
          >
            <div className="relative aspect-[16/10] w-full overflow-hidden bg-secondary">
              ...
              />
              <span className="absolute left-3 top-3 rounded-md bg-primary/95 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-primary-foreground">
                {c.level}
              </span>
            </div>
            <div className="p-4">
              ...
              <div className="mt-4 flex items-center justify-between gap-2">
                ...
                <Button
                  ...
                  onClick={() => toast.success(`Enquiry sent to ${c.coach}`)}
                >
                  Enquire
                </Button>
              </div>
