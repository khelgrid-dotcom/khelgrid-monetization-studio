...
export const Route = createFileRoute("/correction-policy")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "How to report inaccurate, expired or misleading sports opportunity information on KhelGrid.",
      },
      { property: "og:title", content: "KhelGrid Correction Policy" },
      { property: "og:description", content: "How KhelGrid reviews reports and updates sports opportunity information." },
    ],
...
function CorrectionPolicyPage() {
  return (
    <main className="mx-auto max-w-4xl px-4 py-10 md:px-6 md:py-14">
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Trust and safety</p>
      <h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">Correction policy</h1>
      <p className="mt-5 max-w-3xl text-lg leading-relaxed text-muted-foreground">
        Sports opportunities change quickly. KhelGrid welcomes reports from athletes, parents, organizers and coaches so
        that pages remain useful without hiding uncertainty.
      </p>
      ...
      <section className="mt-8 rounded-2xl border border-border bg-card/50 p-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          We may update the visible detail, add a correction note, change the source status, pause the listing, or remove
          it. We do not publish private reporter information without permission. If a correction affects a safety or
          payment decision, users should still confirm the latest position directly with the organizer.
        </p>
      </section>
      ...
      <section className="mt-8 rounded-2xl border border-primary/25 bg-primary/5 p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Use the support channel for listing corrections and suspicious opportunity reports.</p>
        <a href="mailto:support@khelgrid.com" className="mt-4 inline-flex text-sm font-semibold text-primary hover:underline">support@khelgrid.com</a>
      </section>
    </main>
  );
}
