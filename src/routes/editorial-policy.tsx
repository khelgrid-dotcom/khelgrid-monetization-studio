  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-10 md:px-6 md:py-14">
      ...
      <h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">
        KhelGrid editorial policy
      </h1>
      <p className="mt-5 max-w-3xl text-lg leading-relaxed text-muted-foreground">
        KhelGrid publishes sports opportunity information and practical guidance for athletes,
        parents, coaches and organizers. This policy explains how we aim to make those pages useful,
        source-aware and honest about their limits.
      </p>
      ...
      <section className="mt-8 rounded-2xl border border-border bg-card/50 p-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          Software may help us organize catalogs, detect duplicate information, or draft internal
          summaries. Automated output is not treated as a source by itself. A page must be checked
          for accuracy, usefulness, attribution and safety before it is presented as KhelGrid
          guidance. We do not claim that a tool, score, or prediction can select an athlete or
          replace a qualified coach, medical professional, federation rule, or official organizer
          notice.
        </p>
      </section>
      ...
      <section className="mt-8 rounded-2xl border border-primary/25 bg-primary/5 p-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          Send the page URL, the specific statement, the correct information, and a reliable source
          when available. For opportunity corrections, include the event date and organizer so we
          can investigate the right listing.
        </p>
        <Link
          to="/contact"
          className="mt-4 inline-flex text-sm font-semibold text-primary hover:underline"
        >
          Contact the KhelGrid team →
        </Link>
      </section>
    </main>
  );
