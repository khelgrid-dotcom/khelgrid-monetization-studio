...
export const Route = createFileRoute("/play")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Join casual games near you, match by skill level, host your own game.",
      },
    ],
  }),
  component: PlayPage,
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Casual pickup games near you, matched by skill.
              </p>
            </div>
            <Button
              className="rounded-full bg-gradient-hero text-primary-foreground hover:opacity-95"
              onClick={() => toast.success("Host a game flow coming soon")}
            >
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {list.map((g) => {
              ...
              return (
                <div
                  key={g.id}
                  className="rounded-2xl border border-border bg-gradient-card p-4 transition-all hover:-translate-y-0.5"
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="flex items-center gap-2">
                        <Badge className="bg-primary/15 border-primary/40 text-primary">
                          {g.sport}
                        </Badge>
