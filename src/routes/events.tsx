...
function EventsPage() {
  ...
  const list = useMemo(
    () =>
      EVENTS.filter(
        (e) => (city === "All" || e.city === city) && (sport === "All" || e.sport === sport),
      ),
    [sport, city],
  );
    ...
    <main className="mx-auto max-w-7xl px-4 py-6 sm:py-10">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Open formats, leagues and weekend showdowns across India.
        </p>
      </div>

      <div className="mt-4 flex flex-wrap gap-2">
        <Select value={city} onValueChange={setCity}>
          <SelectTrigger className="h-10 w-[160px] bg-secondary/40 border-border">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {list.map((e) => (
          <div
            key={e.id}
            className="overflow-hidden rounded-2xl border border-border bg-gradient-card transition-all hover:-translate-y-0.5"
          >
            ...
            <div className="p-4">
              ...
              <div className="mt-4 flex items-center justify-between gap-2">
                <div className="text-sm font-semibold">
                  ₹{e.entryFee} <span className="font-normal text-muted-foreground">entry</span>
                </div>
                <Button
                  size="sm"
